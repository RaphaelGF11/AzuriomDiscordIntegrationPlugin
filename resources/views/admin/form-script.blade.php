...
@section('title', trans('discord-integration::admin.forms.script_title'))

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h1 class="h3 mb-0">{{ trans('discord-integration::admin.forms.script_editing', ['name' => $form->name]) }}</h1>
        </div>
    ...
    <div class="alert alert-info">
        {{ trans('discord-integration::admin.forms.script_help') }}
    </div>

    <form method="POST" action="{{ route('discord-integration.admin.forms.script.update', $form) }}" id="scriptForm">
        @csrf
        @method('PUT')
        <input type="hidden" name="script" id="scriptHidden" value="{{ old('script', json_encode($form->script ?? [])) }}">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const editor = ScriptEditor.init({
                    ...
                    commandOptions: @json(collect($form->fields ?? [])->pluck('field_key')->filter()->values()),
                    labels: {
                ...
                document.getElementById('scriptExportBtn').addEventListener('click', function () {
                    ...
                    link.href = url;
                    link.download = @json($form->name).replace(/[^a-z0-9_-]/gi, '_') + '-form-script.json';
                    link.click();
